...
export function HeroSection() {
  return (
    <section className="relative flex min-h-[100svh] items-center justify-center overflow-hidden bg-background pt-16 sm:pt-20">
      {/* Gradient orbs */}
      <div className="pointer-events-none absolute inset-0">
        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.15, 0.1] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
          className="absolute left-1/4 top-1/4 h-48 w-48 rounded-full bg-primary/10 blur-[80px] sm:h-72 sm:w-72 sm:blur-[100px] lg:h-96 lg:w-96 lg:blur-[120px]"
        />
        <motion.div
          animate={{ scale: [1, 1.3, 1], opacity: [0.05, 0.1, 0.05] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="absolute bottom-1/4 right-1/4 h-40 w-40 rounded-full bg-primary/5 blur-[60px] sm:h-56 sm:w-56 sm:blur-[80px] lg:h-72 lg:w-72 lg:blur-[100px]"
        />
      </div>
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 text-center sm:px-6">
        ...
        >
          <h1 className="text-3xl font-bold leading-tight tracking-tight text-foreground sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl" style={{ fontFamily: "'Fredoka', 'Baloo 2', cursive, sans-serif", perspective: "800px" }}>
          ...
          <h2 className="mt-3 text-xl font-normal text-foreground sm:mt-4 sm:text-2xl md:text-3xl lg:text-4xl" style={{ fontFamily: "'Fredoka', 'Baloo 2', cursive, sans-serif" }}>
        ...
        >
          <a
            ...
            className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-[0_0_30px_oklch(0.55_0.18_155/0.3)] transition-all hover:shadow-[0_0_50px_oklch(0.55_0.18_155/0.5)] hover:scale-105 sm:w-auto sm:px-8 sm:py-4 sm:text-base"
          >
        ...
        >
          ...
          <div className="mt-4 flex flex-wrap items-center justify-center gap-4 sm:mt-6 sm:gap-8">
            {trustedCompanies.map((company) => (
              <span key={company} className="text-sm font-bold text-muted-foreground/40 transition-colors hover:text-muted-foreground/60 sm:text-lg">
                {company}
              </span>
            ))}
          </div>
